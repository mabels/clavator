...
git clone /gnupg.git /clavator/gnupg-$arch.tmp
VERSION=$(cd /gnupg.git && git rev-parse --verify --short HEAD)
echo ARCH=$arch
...
git clone /gnupg-clavator.git /clavator/gnupg-$arch
mkdir -p /clavator/gnupg-$arch/git/gnupg
git clone --bare /gnupg.git /clavator/gnupg-$arch/git/gnupg
echo $VERSION > /clavator/gnupg-$arch/.VERSION
echo $arch > /clavator/gnupg-$arch/.ARCH
sed -i.orig "s|source=(\"git://github.com/mabels/gnupg.git|source=(\"git+file:///clavator/gnupg-$arch/git/gnupg|" /clavator/gnupg-$arch/PKGBUILD
...
pacman -Syyu --noconfirm base-devel openssh openssl git gcc autoconf make wget base-devel \
  libpng python2 pcsclite imagemagick mesa-libgl librsvg fig2dev ghostscript texinfo patch
...
#docker pull ${DOCKER_REGISTRY}clavator-docker-archlinux-$arch-$DOCKERVERSION
docker run -i --privileged \
   -v /var/cache/docker/clavator:/clavator \
   -t ${DOCKER_REGISTRY}clavator-docker-archlinux-$arch-$DOCKERVERSION \
