...
void sample_authorized_keys(const char *fname) {
  std::ofstream myfile;
  myfile.open (fname);
  myfile <<"# comment\n";
  myfile << "     # comment\n";
...
int main() {
  describe("ssh_authorized_keys", []() {
    it("read ssk_authorized_keys", []() {
      ...
      assert.equal(memcmp(skeys.get()[2].from_data_funny.c_str(), "\1\0\1", 3), 0, "from-funny");
      ...
      assert.equal(skeys.get()[3].from_data_funny, skeys.get()[2].from_data_funny);
