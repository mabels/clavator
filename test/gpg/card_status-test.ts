...
function gpg_card_funny(): string {
  return [
    ...
    'sigcount:16:::\n',
    'cafpr:1:2:3:\n',
    'fpr:F78D5B547A9BB0E8A174C0F5060FF53CB3A32992:B3B94966DF73077EFA734EC83D851A5DF09DEB9C',
...
describe('gpg_card_status', () => {
  it('read gpg_card_status', () => {
    ...
    for (let mycs of css) {
      ...
      assert.equal(mycs.reader.aid, 'AID');
      assert.equal(mycs.reader.cardid, 'D2760001240102010006041775630000');
      assert.equal(mycs.reader.type, 'openpgp-card');
      assert.equal(mycs.version, '0201');
      assert.equal(mycs.vendor, '0006:Yubico');
      assert.equal(mycs.serial, '04177563');
      assert.equal(mycs.name, 'Meno Abels');
      assert.equal(mycs.lang, 'en');
      assert.equal(mycs.sex, 'm');
      assert.equal(mycs.url, 'http://www.geo.de');
      assert.equal(mycs.login, 'abels');
      assert.equal(mycs.forcepin, '0::');
      assert.equal(mycs.keyStates.length, 3);
      const fprs: string[] = [
